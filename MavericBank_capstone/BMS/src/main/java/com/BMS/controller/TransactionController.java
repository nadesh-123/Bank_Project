...
public class TransactionController {
    ...
    public TransactionViewDtoV2 transactionFilter(Principal principal, @RequestParam(required = false) TransactionStatus status, @RequestParam(required = false) TransactionType type,

            @RequestParam(required = false) String sourceAccount, @RequestParam(required = false) String targetAccount,
        ...
        return transactionService.transactionFilter(principal.getName(), status, type, sourceAccount, targetAccount, minAmount, maxAmount, startDate, endDate, page, size);
    }
}
